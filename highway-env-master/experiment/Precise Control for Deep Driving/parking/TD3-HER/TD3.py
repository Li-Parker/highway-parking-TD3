...
class ReplayBuffer:
    ...
    def sample_buffer(self):
        ...
        # the resulting batch will contain self.batch_size number of unique random integers from 0 through mem_len - 1,
        # which can be used as indices in another array or tensor

        # replace=false: specifies that the sampling should be done without replacement, i.e., each element can only
        # occur once in the batch.
        batch = np.random.choice(mem_len, self.batch_size, replace=False)
