...
from TD3withHER import Trajectory
...
parser.add_argument('--figure_file', type=str, default='./output_images_HER/reward.png')
...
def main():
    ...
    env.config["vehicles_count"] = 4  # 停放车辆
    # 设置 Wr 参数
    ...
    # "collision_reward"
    env.config["collision_reward"] = -45
    # env.config["manual_control"] = True
    print(env.config)
    viz = Visdom()
    viz.line([[-60, -60]], [0.], win='TD3-HER-parking_v0', opts=dict(title='TD3-HER-parking_v0'))
    """
    ...
    for episode in range(args.max_episodes):
        ...
        while (not done) and (not done_):
            ...
            traj.store_step(observation, action, reward, observation_, done)
        ...
        viz.line([[total_reward, avg_reward]], [episode + 1], win='TD3-HER-parking_v0', update='append')
